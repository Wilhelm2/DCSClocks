...
nbNodes 	= [2000] # [150,175,200,225,250,300]
clockSize	= [50]#25,50,75] #[100,150,200]

delaySend = 1150
PEAKSPERSECOND = int(nbNodes[0])

subprocess.check_call(["bash","-c","mkdir -p Graphs/dataFiles"])
...
#		subprocess.check_call(["bash","-c","./out/gcc-debug/src/DynamicClockSet_dbg -f ../codeGraphes/omnetpp.ini -n src -u Cmdenv > /dev/null" ], cwd='../codeDCS')	
		...
		subprocess.check_call(["bash","-c","mkdir -p dataFiles/" + str(n) + "_" + str(c)])
		#subprocess.check_call(["bash","-c","cp -r ../codeDCS/data/* dataFiles/" + str(n) + "_" + str(c)  ])
